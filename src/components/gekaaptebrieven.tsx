	...
	components[td.extractor.selector] = function(props) {
		return (
			<Rs
				...
				color={td.color}
				onClick={() => props.activeTab === TabName.TextData ? props.setActiveId(td.id, props.id) : null}
			/>
		)
	}
})
...
components['ner[type="per"]'] = function(props) {
	console.log(props)
	const active = props.activeListId === 'person' && props.activeId === props.children[0]
	return (
		<Rs
			{...props}
			active={active}
			color={personConfig.color}
			onClick={() => props.activeTab === TabName.TextData ? props.setActiveId(personConfig.id, props.id) : null}
		>
			{
				props.activeTab === TabName.TextData &&
				<svg viewBox="0 0 64 64" style={{width: 20, height: 20, verticalAlign: 'text-top'}}>
					<path
						fill={active ? 'white' : personConfig.color}
...
components['ner[type="loc"]'] = function(props) {
	const active =  props.activeListId === 'loc' && props.activeId === props.children[0]
	return (
		<Rs
			{...props}
			active={active}
			color={placeConfig.color}
			onClick={() => props.activeTab === TabName.TextData ? props.setActiveId(placeConfig.id, props.children[0]) : null}
		>
			{
				props.activeTab === TabName.TextData &&
				<svg viewBox="0 0 512 512" style={{width: 20, height: 20, verticalAlign: 'text-top'}}>
					<path
						fill={active ? 'white' : placeConfig.color}
